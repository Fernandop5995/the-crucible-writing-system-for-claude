# Forge Point 0: Ignition — The Council's Confession

> **Provenance**: Plain text = canon from `source-roadmap.md`. **[PROPOSED]** =
> assistant suggestion. **[TBD]** = needs author input.

**Chapter:** 5 (second half) into 6 **[PROPOSED placement]**

**Function (Crucible Structure):** Threshold destroyed; no return.

## Why This Is the Ignition **[PROPOSED]**

The Council meeting is where all three strands collide for the first time and
the door closes behind the trio:

- **Quest:** The poisoned contract lands. They neither accept nor refuse — but
  they now know about the Wyrshade disaster, the voice, and the Vel'Era
  expedition. Knowledge is the threshold; they can't un-know it.
- **Fire:** Elaine's mark opens at the chamber threshold and reads the full
  Thirteen. She recognizes Kymancy and the Throng's signature — and begins the
  book's pattern of carrying unsayable knowledge.
- **Constellation:** The three process one horror three different ways
  (Brunor's stillness, Frederick's arithmetic, Elaine's silence), and Ch 6
  immediately tests the bond through separation.

By the end of Chapter 6 the roadmap states the point-of-no-return outright:
"The Glintgate arc has its teeth in them."

## Canon Scene Material (Ch 5b, beats 1-6)

1. **The Chamber** — one-week timeskip; a secured chamber, not the sanctum,
   not the Archive, not Aedric Spire; the full Thirteen in identical Caltheran
   robes projecting unity; no explanation for the travel; the mark bleeds
   fear/resentment/buried guilt through the performance; Tauvor owns the room.
2. **The Confession** — the Wyrshade vivisection; the illusion; fifteen
   researchers hit with visions none were built to survive; the Rift; the
   rune; all went mad. Elaine recognizes Detect Memory and keeps it to
   herself. Not all Thirteen were told equally.
3. **The Butchery** — the researchers turned on each other; wounds that should
   have been fatal meant nothing; mages destroyed them with fire. Elaine
   reads it: the hive mind. The Council has no name for it. She does, and
   can't say it.
4. **The Voice** — a burning researcher spoke in no one's voice, in a language
   nobody recognized; transcribed phonetically; untranslated. The voice means
   something heard them. Something answered.
5. **The Pivot** — First Era history; the Archive's oldest records; the tongue
   may connect to something old that sleeps in the deep forests of Vel'Era.
6. **The Contract** — escort and guard researchers into Vel'Era. No mention of
   the drake. Enough horror to make the stakes real; enough withheld to hide
   what they'd walk into. The trio leaves carrying questions and an offer that
   smells wrong but pays well.

## What Is Destroyed

- The illusion that the summons was ordinary work
- The Council's standing as a trustworthy employer (they got fifteen of their
  own slaughtered and are still managing the narrative)
- **[TBD]** What acceptance of the contract will cost — and what refusing it
  would cost

## Open Question

The Archwarden's Silence: why the full Thirteen traveled to Glintgate is
deliberately unanswered (unsafe Archive? distance from the sanctum? Order
influence?). Seed, don't resolve.
